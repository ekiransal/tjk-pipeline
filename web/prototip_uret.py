...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .tlink{color:inherit;text-decoration:none;border-bottom:1px dotted #7aa08f;cursor:pointer}
  .tlink:hover{color:#1e6f5c;border-bottom-color:#1e6f5c}
  .tlink .tply{font-size:8px;color:#1e6f5c;margin-left:2px}
  .tlink .tno{font-size:9px;color:#1e6f5c;font-weight:700;margin-left:2px}
...
function detayHTML(b, tab){
  ...
  // DERECE'den hemen sonra: P50 · P66 · P75 · Yarıştaki Seyri · Kaçıncı, sonra kalanlar
  {
    ...
    tasi(kacinci, FK.drc);   // Kaçıncı -> Tarih'in hemen sağı
    const zincir=[...domList.filter(c=>!domKilo.has(c)), seyA];   // Derece'den sonra: P50/P66/P75 + Seyir
  ...
  const th=dolu.map(c=>{
    ...
    const adH=(c===seyA)?"O Tarihteki<br>Seyri":esc(ad);   // Seyir başlığı iki satır (dar kalsın)
  ...
  const trs=rows.map(r=>{
    ...
    else if(tab==="Sayfa1"&&c===19){ cls="orta drc"; if(/^\d+$/.test(v)) v=v+"."; }   // Kaçıncı: 2. 3. gibi
    else if(tab==="Sayfa2"&&c===18){ cls="orta drc"; v="1."; }                        // S8 Kaçıncı: hepsi 1. (kazananın son 800'ü)
    ...
    return `<td class="${cls}${mg2}" data-c="${c}" data-v="${esc(dv)}">${_vhtml||esc(fmt(v))}</td>`;
  }).join("")+"</tr>";}).join("");
