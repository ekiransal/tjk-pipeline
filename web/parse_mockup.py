    ...
    for bi, rs in enumerate(starts):
        ...
        for r in range(rs + 1, re_ + 1):
            ...
            if gal:
                for c0, nm in sorted(gal.items()):
                    ...
                    while rr <= sinir:                       # detaya asla sarkmaz
                        ...
                        rr += 1
                    _gad = str(nm).strip()
                    if "KENTER" in _gad.upper():
                        _gad = "KENTER"
                        _nr = []
                        for _r in rows:
                            _pp = [x.strip() for x in str(_r[0] if _r else "").split("/")]
                            _at = _pp[0] if _pp else ""
                            _tar = _r[1] if len(_r) > 1 else ""
                            if len(_pp) > 1 and TARIH_RE.match(_pp[1]) and not _tar:
                                _tar = _pp[1]
                            _seh = _r[2] if len(_r) > 2 else ""
                            if _at:
                                _nr.append([_at + " / KENTER", _tar, _seh])
                        rows = _nr
                        if not rows:
                            continue
            ...
            if eski_birlesik or eski_devam:
                ...
                blok["detay"].append(
                    ["" if grid.get((r, c)) is None else str(grid.get((r, c))) for c in range(1, 45)])
